...
def library_browser_image(browser_root: str) -> flask.Response:
    selected_root = _library_browser_root_get(browser_root)
    try:
        path, mimetype = rainwave_library.models.storage.library_browser_image_file_get(
            app.config["LIBRARY_ROOT"],
            selected_root,
            flask.request.args.get("path", ""),
        )
...
@app.route("/library-files/<browser_root>/image-preview", methods=["GET"])
@secure
def library_browser_image_preview(browser_root: str) -> str:
    selected_root = _library_browser_root_get(browser_root)
    relative_path = flask.request.args.get("path", "")
    try:
        rainwave_library.models.storage.library_browser_image_file_get(
            app.config["LIBRARY_ROOT"],
            selected_root,
            relative_path,
        )
        ...
        flask.abort(404)
    return rainwave_library.components.library_browser_image_preview(
        selected_root,
        relative_path,
    )
...
def suggestion_assign(suggestion_id: str) -> werkzeug.Response | str:
    ...
    try:
        ...
        if (
            ...
            or (is_assigned and suggestion.status != "claimed")
        ):
            flask.abort(
                409,
                "Only claimed suggestions or unassigned new, accepted, completed, "
                "or declined suggestions can be assigned.",
            )

        excluded_staff_id = None
        if suggestion.status == "new":
            excluded_staff_id = actor_discord_id
        elif suggestion.status == "claimed":
            excluded_staff_id = suggestion.claimed_by_discord_id
        staff_users = rainwave_library.models.storage.staff_users_get(
            storage_cnx,
            exclude_discord_id=excluded_staff_id,
        )
        if flask.request.method == "GET":
